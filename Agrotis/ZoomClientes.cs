...
namespace Agrotis
{
    public partial class ZoomClientes : Form
    {
        String tabela;
        public Object retorno;
        public string cod;
        public ZoomClientes(String tabela)
        {
            this.tabela = tabela;
            InitializeComponent();
            FiltraDados();
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            using (var context = new Contexto())
            {
                if (dataGridViewResult.SelectedRows.Count > 0)
                {
                    ...
                    {                    
                        cod = row.Cells[codigo_cliente.DataPropertyName].Value.ToString();
        ...
        private void FiltraDados()
        {
            using (var context = new Contexto())
            {
                if (tabela == "cliente")
                {
                    var result = context.Cliente.Where(s => s.nome.Contains(edtFiltro.Text) || edtFiltro.Text == "").ToList();
                    dataGridViewResult.DataSource = result;
                }
